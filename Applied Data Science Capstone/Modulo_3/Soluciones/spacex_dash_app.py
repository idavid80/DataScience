...
optionsLaunch = [{'label': 'All Sites', 'value': 'ALL'}] + [{'label': site, 'value': site} for site in launch_sites]
...
def get_pie_chart(entered_site):
    ...
    # Count the success (class=1) and failure (class=0)
    ...
    fig = px.pie(
        ...
        title=f'Success Launches for Site: {entered_site}' if entered_site != 'ALL' else 'Total Success Launches for All Sites'
    )
...
def update_scatter_chart(selected_site, payload_range):
    ...
    # Further filter by selected site if a specific site is chosen
